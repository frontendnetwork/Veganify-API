...
const inputFile = path.join(import.meta.dirname, filename);
const outputFile = path.join(import.meta.dirname, filename);
...
try {
  ...
  console.log(
    `Processing complete. ${processedIngredients.length} ingredients saved in ${outputFile}`
  );
} catch (error) {
