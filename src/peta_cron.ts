...
module.exports = function (app: Application): void {
  ...
  app.get('/v0/cron/peta', async (req: Request, res: Response) => {
    try {
      ...
      array.forEach((entry: string) => {
        if (entry.endsWith('FL')) {
          if (entry.slice(0, -2)) { output.PETA_DOES_NOT_TEST.push(entry.slice(0, -2)) }
        } else if (entry.endsWith('L')) {
          if (entry.slice(0, -1)) { output.PETA_DOES_NOT_TEST.push(entry.slice(0, -1)) }
